...
EOF
  ...
  # comillas simples: sin ellas el sh remoto rompe con paréntesis y otros metacaracteres
...
wait_stream() { # espera fin de stream por logcat; $1=timeout_s
  ...
  while [ $elapsed -lt "$1" ]; do
    ...
    n=$(adb logcat -d --pid=$(adb shell pidof $PKG) 2>/dev/null | grep -c "'data'," || true)
    [ -z "$n" ] && n=0
    [ "$n" -gt 0 ] && started=1
...
# ---- flujo principal: N preguntas desde QUESTIONS_JSON ----
QUESTIONS_JSON="${QUESTIONS_JSON:-/tmp/topk_test_questions.json}"

echo "== unlock + launch =="
adb shell svc power stayon true 2>/dev/null
# lock portrait: el auto-rotate se reactiva solo y rompe el onboarding en landscape
...
sleep 8
# la app re-habilita auto-rotate al lanzarse: re-aplicar el lock DESPUÉS del launch
adb shell settings put system accelerometer_rotation 0
adb shell settings put system user_rotation 0
sleep 2
...
for i in $(seq 0 $((N-1))); do
  ...
  TAG="q$((i+1))"
  if ! adb shell pidof $PKG >/dev/null 2>&1 || [ -z "$(adb shell pidof $PKG)" ]; then
    echo "CRASH antes de $TAG — fin"; break
  fi
  echo "== $TAG =="
